...
class GlobalLock {
  ...
  static GlobalLock create(
    ...
    ArgumentError.checkNotNull(expiration, 'expiration');

    if (dbService == null) {
      throw StateError(
        'The dbService from package:gcloud must be configured in the current service scope',
      );
    }

    return GlobalLock._(lockId, expiration, dbService);
  }
...
class GlobalLockClaim {
  ...
  GlobalLockClaim._(this._entry, this._expiration, this._db) {
    ArgumentError.checkNotNull(_entry, '_entry');
    ArgumentError.checkNotNull(_expiration, '_expiration');
    ArgumentError.checkNotNull(_db, '_db');
  }
