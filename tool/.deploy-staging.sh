...
if [[ "$BRANCH_NAME" != 'master' ]]; then 
  echo 'This script is only intended for use on master branch (for now)'
  exit 1;
fi
...
IMAGE="gcr.io/dartlang-pub-dev/$BRANCH_NAME-staging"

echo "### Building docker image: $IMAGE"
############################################time -p gcloud --project "$PROJECT_ID" builds submit -t "$IMAGE"

APP_VERSION='jonasfj' # Consider using $BRANCH_NAME
