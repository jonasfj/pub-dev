...
class BlobIndex {
  ...
  ///
  /// Throws [FormatException] if this [BlobIndex] is invalid.
  String get blobId {
  ...
  ///
  /// Throws [FileRangeNotFoundException] if [path] could not be found in this
  /// indexed-blob.
  ///
  /// Throws [FormatException] if this [BlobIndex] is invalid.
  FileRange lookup(String path) {
    ...
    for (var i = 0; i < segments.length - 1; i++) {
      if (!_skipUntilKey(r, segments[i]) || !r.tryObject()) {
        throw FileRangeNotFoundException._(path, blobId);
      }
    }
...
/// Thrown when [BlobIndex.lookup] is unable to find an entry for a given path.
@sealed
class FileRangeNotFoundException implements Exception {
  final String _filePath;
  final String _blobId;

  FileRangeNotFoundException._(this._filePath, this._blobId);

  @override
  String toString() =>
      'Could not find "$_filePath" in BlobIndex for blob-id: "$_blobId"';
}
